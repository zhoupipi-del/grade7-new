...
def cyan(t):   print(f"\033[96m{t}\033[0m")
def bold(t):   print(f"\033[1m{t}\033[0m")
...
def test_lesson_prep(tokens, uids):
    ...
    plan_data = {
        ...
        "content": {
            ...
            "teaching_process": [
                {"phase": "导入", "duration": 5, "content": "复习等式性质，引出方程概念", "activities": ["提问", "回顾"], "resources": ["PPT"]},
                {"phase": "新授", "duration": 20, "content": "讲解移项法则和合并同类项", "activities": ["讲授", "板演"], "resources": ["教材", "微课"]},
                {"phase": "练习", "duration": 15, "content": "分层练习题", "activities": ["独立练习", "同桌互批"], "resources": ["练习单"]},
                {"phase": "小结", "duration": 3, "content": "归纳解方程步骤", "activities": ["学生总结"], "resources": []},
                {"phase": "作业", "duration": 2, "content": "课本P102 练习1-5题", "activities": [], "resources": []}
            ],
    ...
    v2_content = {
        ...
        "teaching_process": [
            {"phase": "导入", "duration": 5, "content": "情境引入: 鸡兔同笼问题", "activities": ["情境创设", "提问"], "resources": ["PPT", "动画"]},
            {"phase": "新授", "duration": 25, "content": "讲解移项、合并同类项、系数化为1，含分母方程示范", "activities": ["讲授", "板演", "小组讨论"], "resources": ["教材", "微课", "几何画板"]},
            {"phase": "练习", "duration": 10, "content": "分层练习: 基础题+提高题", "activities": ["独立练习", "上台展示"], "resources": ["练习单"]},
            {"phase": "小结", "duration": 3, "content": "归纳解方程四步法", "activities": ["学生总结"], "resources": []},
            {"phase": "作业", "duration": 2, "content": "基础题+拓展题分层布置", "activities": [], "resources": []}
        ],
        "homework": ["课本P102 练习1-5题(基础)", "拓展: 实际问题建模1题", "预习: 二元一次方程组"],
        "blackboard_design": "一元一次方程解法\\nStep1: 去分母\\nStep2: 去括号\\nStep3: 移项\\nStep4: 合并同类项\\nStep5: 系数化为1",
        "reflection": ""
    }
    resp = api("POST", f"research_lesson_prep/{plan_id}/versions", t_token, {
        "content": v2_content, "change_log": "增加化归思想目标, 新增情境导入, 细化分层练习", "is_major": True
    })
    if check(resp, "创建V2"):
        yellow(f"    version_number={resp.json()['version_number']}, is_major={resp.json()['is_major']}")
    ...
    reviews = [
        {"version_number": 2, "target_section": "teaching_objectives", "target_anchor": "item_3",
         "comment": "第三个目标'将实际问题转化为方程'建议拆分为更具体的能力指标，如'能从行程问题中提取等量关系'", "severity": "suggestion"},
        {"version_number": 2, "target_section": "teaching_process", "target_anchor": "step_2",
         "comment": "新授环节25分钟偏长，建议压缩至20分钟，将节省的时间用于练习环节的师生共评", "severity": "issue"},
        {"version_number": 2, "target_section": "homework", "target_anchor": "item_2",
         "comment": "拓展题'实际问题建模'缺少梯度支架，初一学生直接建模困难较大，建议增加中间引导步骤", "severity": "critical"},
    ]
    review_ids = []
    ...
    if review_ids:
        resp = api("PUT", f"research_lesson_prep/{plan_id}/reviews/{review_ids[0]}", l_token,
    ...
    if check(resp, "发布"):
        yellow(f"    status={resp.json()['status']}, published_version={resp.json().get('published_version')}")

    # 1i. 教师Fork已发布教案
    cyan("\n[1i] 教师 Fork 已发布教案...")
    resp = api("POST", f"research_lesson_prep/{plan_id}/fork", t_token,
               {"title": "七年级数学《一元一次方程》二次备课(个性化修改)"})
    if check(resp, "Fork派生"):
        forked = resp.json()
        yellow(f"    forked_plan_id={forked['id']}, forked_from_id={forked.get('forked_from_id')}, status={forked['status']}")
    ...
    if check(resp, "版本历史"):
        ...
        for v in versions:
            ...
            yellow(f"      V{v['version_number']}: {v.get('change_log','?')}{tag} (by {v.get('editor_name','?')})")
    ...
    if check(resp, "Dashboard"):
        d = resp.json()
        yellow(f"    total={d['total_plans']}, draft={d['draft_count']}, review={d['review_count']}, approved={d['approved_count']}, published={d['published_count']}")
...
def test_observation_appeal(tokens, uids, plan_id):
    ...
    obs = resp.json()
    obs_id = obs["id"]
    yellow(f"    obs_id={obs_id}, feedback_status={obs['feedback_status']}, lesson_plan_id={obs.get('lesson_plan_id')}")
    ...
    rubric_data = {
        ...
        "dimensions": [
            {"name": "教学目标", "score": 8.5, "max": 10, "weight": 0.15, "comment": "目标明确, 但化归思想渗透不够"},
            {"name": "教学内容", "score": 9.0, "max": 10, "weight": 0.20, "comment": "内容准确, 重点突出"},
            {"name": "教学方法", "score": 7.5, "max": 10, "weight": 0.15, "comment": "讲授为主, 互动偏少"},
            {"name": "教学过程", "score": 7.0, "max": 10, "weight": 0.20, "comment": "时间分配欠佳, 导入超时"},
            {"name": "教学效果", "score": 8.0, "max": 10, "weight": 0.15, "comment": "多数学生掌握, 少数需巩固"},
            {"name": "教师素养", "score": 9.5, "max": 10, "weight": 0.15, "comment": "板书规范, 语言表达清晰"}
        ]
    }
    resp = api("POST", f"research_observation/{obs_id}/rubric", l_token, rubric_data)
    if check(resp, "提交评分"):
        r = resp.json()
        yellow(f"    total_score={r['total_score']}/{r['max_score']}, percentage={r['percentage']}%")

    # 2c. 教师申诉 (PENDING → APPEALED)
    cyan("\n[2c] 教师提出申诉 (PENDING → APPEALED)...")
    resp = api("POST", f"research_observation/{obs_id}/appeal", tokens["teacher"], {
    ...
    cyan("\n[2e] 组长处理申诉 (APPEALED → RESOLVED)...")
    resp = api("POST", f"research_observation/{obs_id}/resolve", l_token, {
        "resolution": "经组内讨论, 导入超时确有客观原因(学生预习不足), 教学过程维度评分上调至8.0。教学方法维度维持原评, 小组讨论时长不足5分钟, 互动深度有限。总分调整为81.25分。",
        "score_adjusted": True,
        "adjusted_total_score": 81.25
    })
    if check(resp, "处理申诉"):
        yellow(f"    feedback_status={resp.json()['feedback_status']}, score={resp.json().get('score_total')}")
...
def test_observation_confirm(tokens, uids, plan_id):
    ...
    # 3b. 提交评分
    cyan("\n[3b] 提交评分矩阵...")
    resp = api("POST", f"research_observation/{obs_id}/rubric", l_token, {
        "template_name": "梨江中学复习课评分表",
        "dimensions": [
            {"name": "教学目标", "score": 9.5, "max": 10, "comment": "复习目标明确, 全面覆盖"},
            {"name": "教学内容", "score": 9.0, "max": 10, "comment": "知识梳理系统, 典型题选取精当"},
    ...
    if check(resp, "提交评分"):
        yellow(f"    total={resp.json()['total_score']}/{resp.json()['max_score']}, {resp.json()['percentage']}%")
...
def test_activities(tokens, uids, plan_id, obs_id_appeal, obs_id_confirm):
    ...
    act = resp.json()
    act_id = act["id"]
    yellow(f"    act_id={act_id}, status={act['status']}, participants={act['participant_count']}, agendas={act['agenda_count']}")
    yellow(f"    linked_plans={act.get('linked_plan_ids')}, linked_observations={act.get('linked_observation_ids')}")

    # 4b. 添加议题1 (关联教案)
    cyan("\n[4b] 添加议题1: 备课方案研讨 (关联教案)...")
    resp = api("POST", f"research_activities/{act_id}/agendas", l_token, {
    ...
    if check(resp, "议题1"):
        ...
        yellow(f"    agenda_id={agenda1_id}, seq={resp.json()['seq']}, linked_plan={resp.json().get('linked_plan_id')}")
    ...
    if check(resp, "议题2"):
        ...
        yellow(f"    agenda_id={agenda2_id}, seq={resp.json()['seq']}, linked_observation={resp.json().get('linked_observation_id')}")
    ...
    if check(resp, "获取参与人列表"):
        ...
        for p in participants:
            resp2 = api("PUT", f"research_activities/{act_id}/participants/{p['id']}", l_token, {
    ...
    if check(resp, "获取议题列表"):
        ...
        if agendas:
            resp2 = api("PUT", f"research_activities/{act_id}/agendas/{agendas[0]['id']}", l_token, {
                "actual_duration": 28,
    ...
    cyan("\n[4h] 更新议题2讨论结果...")
    if len(agendas) > 1:
        resp2 = api("PUT", f"research_activities/{act_id}/agendas/{agendas[1]['id']}", l_token, {
            "actual_duration": 30,
    ...
    cyan("\n[4i] 完成活动 (IN_PROGRESS → COMPLETED)...")
    resp = api("PUT", f"research_activities/{act_id}", l_token, {
    ...
    if check(resp, "Dashboard"):
        d = resp.json()
        yellow(f"    total={d['total_activities']}, planned={d['planned']}, in_progress={d['in_progress']}, completed={d['completed']}")
        yellow(f"    participants={d['total_participants']}, agendas={d['total_agendas']}, resolved={d['resolved_agendas']}")
