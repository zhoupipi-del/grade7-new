...
| 文件 | 内容 |
|---|---|
| `wings_contracts/edge/base.py` | `ConnectorStatus` / `HealthStatus` / `FieldDef` / `DataSchema` / `DeltaBatch` / `BaseConnector`（冻结契约） |
| `wings_contracts/edge/protocol.py` | `TaskType` / `TaskStatus` / `EdgeTask` / `TaskResult` / `EdgeHeartbeat`（含 ERRATA-001/002） |
| `wings_contracts/ai/privacy/base.py` | `StudentDataProvider` 抽象（四方法，冻结契约） |
| `wings_contracts/ai/privacy/cloud_provider.py` | `CloudStudentDataProvider` 适配器声明（NOT_WIRED，见下） |
| `wings_contracts/ai/privacy/psych_provider.py` | `PsychDataProvider` 抽象（最小接口，由 CF-01 call site 提取） |
...
from wings_contracts.edge.base import BaseConnector, DataSchema, DeltaBatch, HealthStatus, ConnectorStatus
...
```python
from wings_contracts.edge.base import FieldDef, DataSchema, DeltaBatch
...
from wings_contracts.edge.protocol import EdgeTask, TaskResult, TaskStatus, TaskType, EdgeHeartbeat
from wings_contracts.edge.base import HealthStatus, ConnectorStatus
...
### ERRATA-001（已应用）
`wings_contracts/edge/protocol.py` 补 `from .base import HealthStatus`（原文档代码块缺 import，mypy/pyright 会报错）。
