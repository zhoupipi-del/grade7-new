...
from utils.inference import (
    ...
    invalidate_model_cache as invalidate_inference_cache,
)

bp = Blueprint("ai_inference", __name__, url_prefix="/ai-api")

# ── 年级基线缓存 (FeatureExtractor 特有，非 inference 模块职责) ──
...
def invalidate_model_cache():
    ...
    失效所有内存缓存（含 inference 模块全局缓存 + 本模块基线缓存）。
    """
    global _BASELINES
...
def predict(student_id):
    ...
    try:
        pipeline, metadata = load_xgb_pipeline(model_dir=model_dir)
    ...
    # ③ 推理 — 使用共享 inference 模块
    inference_result = run_inference(features, pipeline, metadata)
    risk_prob = inference_result["risk_prob"]
...
def _get_talk_sheet_data(student_id):
    ...
    evidence = {}
    try:
        model_dir = os.path.join(ctx.root_path, "models")
        pipeline, metadata = load_xgb_pipeline(model_dir=model_dir)
    ...
    return {
        ...
        "risk_prob": risk_prob,
        "risk_level": risk_level,
        "risk_pct": f"{risk_prob * 100:.1f}" if risk_prob is not None else "N/A",
...
def retrain_status():
    ...
    model_mtime = datetime.fromtimestamp(os.path.getmtime(model_path)).isoformat() if model_exists else None

    return jsonify({
        "cache_active": model_exists,
...
def create_intervention():
    ...
    try:
        ...
        result = fe.get_student_vector(student_id)
        if result["features"]:
            inference_result = run_inference(result["features"], pipeline, metadata)
            risk_before = inference_result["risk_prob"]
    except Exception:
        pass
...
def update_followup(int_id):
    ...
    if risk_after is None:
        try:
            ...
            if result["features"]:
                inference_result = run_inference(result["features"], pipeline, metadata)
                risk_after = inference_result["risk_prob"]
        except Exception:
            pass
