...
def predict(student_id):
    ...
    top_factors = [
        {"feature": passed_features[i], "importance": round(float(importances[i]), 4)}
        for i in top2_idx
    ]
...
def _build_evidence(feature_dict: dict, baselines: dict) -> dict:
    ...
    if math_avg < 60:
        ...
        evidence["math"] = f"数学成绩 {math_avg:.1f} 分（略低于年级均值）"
    else:
        evidence["math"] = f"数学成绩 {math_avg:.1f} 分（高于年级均值 {ratio:.2f} 倍）"
    ...
    elif att < att_base - 0.02:
        evidence["attendance"] = f"出勤率 {att_pct:.1f}%（略低于均值）"
    else:
        evidence["attendance"] = f"出勤率 {att_pct:.1f}%（符合年级水平）"
    ...
    if quality < 70:
        ...
        evidence["quality"] = f"综合素质分 {quality:.1f}（年级均值 {quality_base:.1f}）"
...
def create_intervention():
    ...
    rec = InterventionRecord(
        ...
        intervention_date=data.get("intervention_date") or date.today(),
