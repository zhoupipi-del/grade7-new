...
class EvaluationIndicator(Base, SchoolMixin):
    ...
    )  # 维度标识: moral/academic/health/art/social
    ...
    __table_args__ = (Index("idx_ei_dim_active", "school_id", "dimension", "is_active"),)
...
class ScoreLog(Base, SchoolMixin):
    ...
    policy_tag = Column(
        String(20), default="repairable"
    )  # PolicyEngine 标签: repairable/non_repairable/recovered/permanent
    created_by = Column(BigInteger, ForeignKey("users.id"), nullable=True)
    created_at = Column(DateTime, default=get_local_now)

    # ── #1193 血缘增强字段 ──
    actor_id = Column(
        BigInteger, nullable=True, comment="实际操作者（与 created_by 区分，支持委托场景）"
    )
    source_ip = Column(String(45), nullable=True, comment="操作来源 IP（IPv4/IPv6，审计合规）")
    trace_context_id = Column(
        String(36),
        nullable=True,
        index=True,
        comment="血缘追踪上下文 ID（关联 LineageEvent.trace_id）",
    )
    diff_snapshot = Column(
        JSON, nullable=True, comment="变更前后 JSON 对比快照 {before:{...}, after:{...}}"
    )
...
class RecoveryState(Base, SchoolMixin):
    ...
    policy_tag = Column(
        String(20), default="repairable"
    )  # repairable/non_repairable/recovered/permanent
