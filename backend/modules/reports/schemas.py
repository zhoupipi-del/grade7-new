...
class ExportMoralReportRequest(BaseModel):
    ...
    semester: str | None = Field(None, description="学期标识，如 2025-2026-2")
    report_type: str = Field(
        "class_moral", description="报告类型: class_moral / student_individual"
    )
    student_id: int | None = Field(
        None, ge=1, description="指定学生 ID（student_individual 时必填）"
    )
...
class TaskStatusResponse(BaseModel):
    ...
    status_text: str | None = Field(None, description="当前步骤描述")
    result: dict[str, Any] | None = Field(
        None, description="成功时返回 {filename, download_url, ...}"
    )
...
class ExportGradeReportRequest(BaseModel):
    ...
    include_classes: list[int] | None = Field(
        None, description="指定班级 ID 列表（为空则导出全年级）"
    )
...
class RiskStudentSummary(BaseModel):
    ...
    risk_level: str = Field(
        ..., description="风险层级: red_intervention / yellow_attention / green_normal"
    )
...
class ClassTeacherReportResponse(BaseModel):
    ...
    high_risk_students: list[RiskStudentSummary] = Field(..., description="本班高危+关注学生清单")
