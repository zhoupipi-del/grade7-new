...
class DisciplineSanction(Base, SchoolMixin):
    ...
    # ── 核心关联 ──
    student_id = Column(
        BigInteger, ForeignKey("students.id"), nullable=False, index=True, comment="被处分学生"
    )
    class_id = Column(
        BigInteger,
    ...
    behavior_record_id = Column(
        BigInteger,
        ForeignKey("discipline_records.id"),
        nullable=True,
        index=True,
    ...
    # ── 时间线索 ──
    punish_date = Column(
        Date, nullable=False, default=date.today, comment="处分下发日期（审批通过当天）"
    )
    ...
    approver_id = Column(
        BigInteger, ForeignKey("users.id"), nullable=True, comment="审批人（德育主任）"
    )

    # ── 二级审批审计追踪 ──
    grade_leader_id = Column(
        BigInteger, ForeignKey("users.id"), nullable=True, comment="初审人（年级组长）"
    )
    ...
    behavior_record = relationship(
        ...
        foreign_keys=[behavior_record_id],
        lazy="selectin",
    )
...
class SanctionAppeal(Base, SchoolMixin):
    ...
    sanction_id = Column(
        BigInteger,
        ForeignKey("discipline_sanctions.id"),
        nullable=False,
        index=True,
        comment="被申诉的处分记录",
    )
    ...
    idempotency_key = Column(
        String(100), nullable=False, unique=True, index=True, comment="外部系统幂等键，防重复提交"
    )

    # ── 复核状态机 ──
    status = Column(
        SQLEnum(AppealStatus),
        nullable=False,
    ...
    # ── 复核审计 ──
    reviewer_id = Column(
        BigInteger, ForeignKey("users.id"), nullable=True, comment="复核人（德育处管理员）"
    )
