...
from modules.risk_models.models import MentalHealthAssessment, PsychSurvey  # noqa: F401
...
class MentalHealthQuestion(Base, SchoolMixin):
    ...
    scale_name = Column(
        String(100), nullable=False, index=True, comment="量表名称: MSSMHS-55 / SCL-90"
    )
...
class MentalHealthAnswer(Base, SchoolMixin):
    ...
    assessment_id = Column(
        BigInteger, ForeignKey("mental_health_assessments.id"), nullable=False, index=True
    )
    question_id = Column(
        BigInteger, ForeignKey("mental_health_questions.id"), nullable=False, index=True
    )
...
class InterventionRecord(Base, SchoolMixin):
    ...
    teacher_id = Column(
        BigInteger, ForeignKey("users.id"), nullable=False, index=True, comment="干预教师"
    )
    assessment_id = Column(
        BigInteger,
        ForeignKey("mental_health_assessments.id"),
        index=True,
        comment="关联评估 (可选)",
    )
    ...
    effect_rating = Column(
        String(20), index=True, comment="效果评定: 显著好转/略有好转/无变化/恶化"
    )
    ...
    status = Column(
        String(20), nullable=False, default="tracking", comment="tracking / completed / cancelled"
    )
