...
class TeacherExtension(Base, SchoolMixin):
    ...
    hired_at = Column(
        "hire_date", Date, nullable=True, comment="入职日期"
    )  # DB列名hire_date→ORM属性hired_at
    office_location = Column(String(100), nullable=True, comment="办公地点")
    qualifications = Column(
        JSON, nullable=True, comment="资质证书列表 ['教师资格证','心理咨询师',...]"
    )
...
class TeacherWorkload(Base, SchoolMixin):
    ...
    semester = Column(
        String(20),
        nullable=False,
        comment="学期: 2025-2026-1 / 2025-2026-2",
    )
    ...
    extra_duties = Column(
        JSON,
        nullable=True,
        comment='兼任职务: ["年级组长","教研组长","备课组长",...]',
    )
    total_workload_score = Column(
        Float,
        nullable=True,
        comment="工作量综合评分 (自动计算)",
    )
    notes = Column(String(255), nullable=True, comment="备注")
    created_at = Column(DateTime, default=get_local_now)
    updated_at = Column(DateTime, default=get_local_now, onupdate=get_local_now)
