...
def _build_fallback_regulations(policy: dict) -> dict[str, str]:
    ...
    for event_type, event_config in behavior_types.items():
        if event_type in rule_templates:
            ...
            regulations[event_type] = (
                f"违反校规行为 ({event_type})，严重程度: {severity}，基础扣分: {penalty}分。"
            )
...
class PenaltyExplainer:
    ...
    async def generate_explanation(
        ...
        template_name, rendered_text, violations = self._render_template(
            ...
            recommended_action=growth["suggested_actions"][0]
            if growth["suggested_actions"]
            else "观察",
            suppression_reason=rdi_result.get("suppression_reason", "") if rdi_result else "",
            cooldown_hours=str(
                self.policy.get("risk_warning", {})
                .get("warning_suppression", {})
                .get("repeated_warning_cooldown_hours", 48)
            ),
            z_score=z_score,
    ...
    async def _fetch_discipline_record(self, record_id: int) -> dict | None:
        """查询指定违纪记录"""
        try:
            ...
            if record:
                ...
                event_type_from_db = self._CATEGORY_TO_POLICY_KEY.get(
                    raw_category, self._CATEGORY_TO_POLICY_KEY.get(raw_type, raw_type or "unknown")
                )
                return {
                    "id": record.id,
                    ...
                    "event_date": str(
                        getattr(
                            record, "incident_date", getattr(record, "created_at", date.today())
                        )
                    ),
                    "penalty_amount": getattr(record, "points", None),
                    "description": getattr(record, "description", "")
                    or getattr(record, "detail", ""),
    ...
    async def _fetch_latest_discipline_record(
        self, student_id: int, event_type: str | None = None
    ) -> dict | None:
        ...
        try:
            ...
            if event_type:
                ...
                query = query.where(getattr(DisciplineRecord, "behavior_type", None) == event_type)
            ...
            query = query.order_by(
                desc(
                    getattr(
                        DisciplineRecord,
                        "incident_date",
                        getattr(DisciplineRecord, "created_at", datetime.min),
                    )
                )
            ).limit(1)
            ...
            logger.warning(
                f"Failed to fetch latest discipline_record for student_id={student_id}: {e}"
            )
        return None

    async def _fetch_latest_score_log(self, student_id: int) -> dict | None:
        """查询学生最新的评价变动流水"""
        try:
            result = await self.db.execute(
                select(ScoreLog)
    ...
    def _build_rule(self, event_type: str) -> dict:
        ...
        try:
            ...
            regulation_ref = self._regulations.get(
                event_type,
                self._regulations.get(
                    "default", "违反校规行为，按照《梨江中学学生违纪处分条例》处理。"
                ),
            )
            ...
            return {
                ...
                "effective_penalty": float(event_config.get("base_penalty", 5))
                * float(event_config.get("weight_multiplier", 1.0)),
            }
    ...
    def _build_growth(
        ...
        try:
            ...
            if is_repairable:
                # 获取回血参数
                k = severity_config.get(
                    "k_override", recovery_model.get("parameters", {}).get("k", 0.5)
                )
                ...
                recovery_path = (
                    ...
                    else f"保持{min_days}天无违纪行为后进入观察期，随时间自然回血"
                )
            else:
            ...
            if event_type and rdi_result:
                ...
                if risk_level in ("attention", "intervention"):
                    ai_prescription_ref = (
                        f"建议参考 AI 德育处方模块，为 {event_type} 类型事件匹配个性化干预方案"
                    )
    ...
    def _get_suggested_actions(self, rdi_result: dict | None = None) -> list[str]:
    ...
    def _render_template(
        ...
        rendered = rendered.replace(
            "{rdi_score}",
            f"{rdi_score:.2f}" if isinstance(rdi_score, (int, float)) else str(rdi_score),
        )
    ...
    async def _fetch_student_info(self, student_id: int) -> dict | None:
        """获取学生基本信息"""
        try:
            result = await self.db.execute(
                select(
