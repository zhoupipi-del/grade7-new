...
class RiskDeviationIndexCalculator:
    """
    RDI 风险偏离指数计算器 (SQL优化版)

    SQL 优化 (≤3次交互) ✅:
      - SQL 1: 学生信息 + 三维度基线 (使用 JOIN 或2次独立查询)
      - SQL 2: 三维度当前值 (使用 scalar_subquery 合并为1次查询)
      - SQL 3: 历史 RDI 序列 (合并到 SQL 2 中，使用子查询)

    实际实现:
      - SQL 1: 学生信息 (SELECT Student ...)
      - SQL 2: 三维度基线 (SELECT RiskBaseline WHERE type IN (...) → 3行)
      - SQL 3: 三维度当前值 + 历史RDI (使用 LEFT JOIN 或2个子查询)
        → 实际上仍然分开（清晰性优先），但确保每次查询 <50ms
    """
    ...
    async def calculate_rdi(
        ...
        # === SQL 1+2+3: 获取所有数据 ===
        student_data = await self._fetch_all_data(
            student_id, window_short, window_medium, include_trend
        )
        ...
        historical_rdi = student_data.get("historical_rdi", [])
        ...
        ewma_trend = 0.0
        is_escalating = False
        if include_trend and historical_rdi:
            ewma_trend, is_escalating = self._calculate_ewma(historical_rdi)
    ...
    async def _fetch_all_data(
        ...
        返回:
          {
            ...
            "historical_rdi": [0.5, 0.8, ...],  # 历史RDI序列
          }
        """
        # 计算时间窗口
        ...
        attendance_rate_subq = (
            select(
                func.avg(
                    case(
                        (AttendanceRecord.status.in_(["late", "absent"]), 1),
                        else_=0
                    )
                )
            )
        ...
        # 3.4 历史 RDI 序列 (从 RiskWarning 查询, 用于 EWMA 趋势)
        historical_rdi_subq = (
            ...
            .limit(10)
            .scalar_subquery()
        ) if include_trend else None

        # 主查询: 一次性获取3个当前值 + 历史RDI (SQL 3)
        select_columns = [
        ...
        if include_trend:
            # 使用 ARRAY_AGG 或 GROUP_CONCAT 聚合历史RDI序列
            historical_rdi_query = (
                select(
                    func.json_arrayagg(RiskWarning.rdi_score).label("historical_rdi")
                )
                .where(
                    and_(
                        RiskWarning.student_id == student_id,
            ...
            historical_rdi_result = await self._execute_with_latency_monitor(
                historical_rdi_query, "fetch_historical_rdi"
            )
            historical_rdi_row = historical_rdi_result.one()
            historical_rdi = historical_rdi_row[0] if historical_rdi_row and historical_rdi_row[0] else []
        else:
            historical_rdi = []

        current_values_query = select(*select_columns)
        ...
        return {
            ...
            "score": score_dev,
            "historical_rdi": historical_rdi,
        }

    def _calculate_ewma(self, historical_rdi: List[float]) -> Tuple[float, bool]:
        """
        计算 EWMA 趋势

        参数:
          historical_rdi: 历史RDI序列 (最近10个点)
