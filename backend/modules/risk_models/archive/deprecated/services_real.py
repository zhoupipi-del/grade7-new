...
核心功能:
  ...
  - 真实数据源: discipline_records / attendance_records / student_scores
...
class RiskDeviationIndexCalculator:
    ...
    async def _fetch_student_and_deviations(
        ...
        # 3.1 行为维度: 过去 window_short 天违纪次数 (从 discipline_records 查询)
        behavior_count_subq = (
            select(func.count())
            .select_from(text("discipline_records"))
            .where(
                and_(
                    text("discipline_records.student_id = :student_id"),
                    text("discipline_records.incident_date >= :short_start"),
                    text("discipline_records.school_id = :school_id"),
                )
            )
            .scalar_subquery()
        )

        # 3.2 考勤维度: 过去 window_short 天迟到率 (从 attendance_records 查询)
        attendance_rate_subq = (
            ...
            .where(
                and_(
                    text("attendance_records.student_id = :student_id"),
                    text("attendance_records.record_date >= :short_start"),
                    text("attendance_records.school_id = :school_id"),
                )
            )
            .scalar_subquery()
        )

        # 3.3 评价维度: 最新 total_score (从 student_scores 查询)
        score_avg_subq = (
            select(func.max(text("student_scores.total_score")))
            .select_from(text("student_scores"))
            .where(
                and_(
                    text("student_scores.student_id = :student_id"),
                    text("student_scores.school_id = :school_id"),
                )
            )
            .scalar_subquery()
        )
