...
class Notification(Base, SchoolMixin):
    ...
    recipient_id = mapped_column(
        BigInteger, ForeignKey("users.id"), nullable=False, index=True, comment="收件人"
    )
    ...
    # ── 内容 ──
    title = mapped_column(String(200), nullable=False, comment="通知标题")
    body = mapped_column(Text, nullable=True, comment="通知正文")
    ...
    entity_id = mapped_column(
        BigInteger, nullable=True, comment="关联实体 ID, 配合 entity_type 构建跳转链接"
    )

    # ── 状态 ──
    is_read = mapped_column(Boolean, default=False, nullable=False, index=True, comment="是否已读")
    read_at = mapped_column(DateTime, nullable=True, comment="已读时间")

    # ── 时间 ──
    created_at = mapped_column(
        DateTime, default=get_local_now, nullable=False, comment="通知创建时间"
    )
