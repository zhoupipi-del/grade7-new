...
API_PREFIX = "/api/v1/risk_models"  # 注意: 下划线, 非连字符 (匹配 Nginx location 和 module_loader prefix)
...
def validate_three_section_structure(result: dict, scenario: str) -> int:
    ...
    if not rule:
        ...
        required_rule_fields = ["regulation_ref", "severity", "dimension", "base_penalty", "weight_multiplier"]
...
def run_scenario(scenario_key: str, scenario: dict, token: str) -> dict:
    ...
    body = result["body"]
    print_info(f"响应: {elapsed_ms:.1f}ms | student_name={body.get('student_name')} | class={body.get('class_name')}")
    ...
    print_info(f"  Fact: {fact.get('event_type')} | {fact.get('event_date')} | "
               f"penalty={fact.get('penalty_amount')} | source={fact.get('data_source')}")
    print_info(f"  FacT描述: {fact.get('description', '')[:80]}")
    print_info(f"  Rule: {rule.get('severity')} | {rule.get('dimension')} | "
    ...
    print_info(f"  Growth: repairable={growth.get('repairable')} | "
...
def main():
    ...
    print_info(f"  错误处理: {'✅ PASS' if error_errors == 0 else f'❌ FAIL ({error_errors} errors)'}")
