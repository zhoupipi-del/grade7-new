...
DB_CMD = "docker exec -i grade7-new-db mysql -ugrade7 -p'waOPKoyFf4ByQD1h' wings3"
...
result = subprocess.run(
    ['ssh', 'root@8.137.180.152',
     "docker exec grade7-new-db mysql -ugrade7 -p'waOPKoyFf4ByQD1h' wings3 -N -e "
    ...
    if day_offset < 20:
        ...
        for _ in range(num):
            ...
            violation_records.append(
                (stu['id'], stu['class_id'], stu['grade_id'], vtype, cat, desc, pts, date_str, f"{date_str} 10:{random.randint(0,59):02d}:00")
            )
        ...
        if day_offset % 3 == 0:
            stu = random.choice(students)
            violation_records.append(
                (stu['id'], stu['class_id'], stu['grade_id'], 'serious', '打架', '严重打架事件触发滑窗红线', 10, date_str, f"{date_str} 14:30:00")
            )
...
# ── 清空旧测试数据 ──
sql_parts.append("DELETE FROM discipline_records WHERE school_id=1 AND created_by=1 AND verify_status='VERIFIED' AND description LIKE '%烟幕%' OR (school_id=1 AND incident_date >= '2026-05-26');")
sql_parts.append("DELETE FROM student_scores WHERE school_id=1 AND semester='2025-2026-2';")

# ── 批量插入违纪记录 ──
sql_parts.append("INSERT INTO discipline_records (student_id, class_id, grade_id, type, category, description, points, status, verify_status, created_by, school_id, incident_date, created_at) VALUES")
values_parts = []
for r in violation_records:
    ...
    cat_esc = cat.replace("'", "\\'")
    values_parts.append(f"({sid},{cid},{gid},'{vtype}','{cat_esc}','{desc_esc}',{pts},'active','VERIFIED',1,1,'{dstr}','{dtstr}')")
sql_parts.append(',\n'.join(values_parts) + ';')

# ── 批量插入德育分数 ──
sql_parts.append("INSERT INTO student_scores (student_id, class_id, grade_id, semester, moral_score, academic_score, health_score, art_score, social_score, total_score, base_score, school_id) VALUES")
score_values = []
...
verify_result = subprocess.run(
    ['ssh', 'root@8.137.180.152',
     "docker exec grade7-new-db mysql -ugrade7 -p'waOPKoyFf4ByQD1h' wings3 -e "
