...
def verify_school_access(requested_school_id: int, current_user: User) -> int:
    ...
    if user_role == UserRole.MS_ADMIN:
        return requested_school_id  # 超管放行
...
async def verify_entity_ownership(
    ...
    """
    实体归属权校验 — 确保 entity_id 对应的记录属于 current_user 的学校。
    ...
    # 只对有 school_id 列的实体做隔离校验
    if user_role != UserRole.MS_ADMIN and hasattr(entity, "school_id"):
