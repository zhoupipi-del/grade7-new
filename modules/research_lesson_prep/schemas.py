...
class PlanCreate(BaseModel):
    ...
    content_markdown: Optional[str] = Field(None, description="Markdown+LaTeX 正文")
...
class AIBiasGenerateRequest(BaseModel):
    """AI逆向处方生成请求"""
    grade_id: Optional[int] = Field(None, description="年级ID (拉取该年级断层数据)")
    class_id: Optional[int] = Field(None, description="班级ID (拉取该班级断层数据, 优先级高于grade_id)")
...
class PlanResponse(BaseModel):
    ...
    content_markdown: Optional[str] = None
    ai_bias_prescription: Optional[str] = None
