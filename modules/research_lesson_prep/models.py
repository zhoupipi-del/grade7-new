...
class ResearchLessonPlan(Base, SchoolMixin):
    ...
    content_markdown = Column(
        Text,
        comment="Markdown+LaTeX 教案正文 (协同编辑的完整文本内容)"
    )

    # ── AI学情逆向处方 (Wings 3.1 从error_funnel逆向注入) ──
    ai_bias_prescription = Column(
        Text,
        comment="AI学情逆向处方 (DeepSeek从错题断层逆向生成的教学偏方)"
    )
    ai_prescription_generated_at = Column(
        DateTime, comment="AI处方最后生成时间"
    )
...
class ResearchPlanVersion(Base, SchoolMixin):
    ...
    content_markdown = Column(
        Text,
        comment="Markdown+LaTeX 正文快照 (每次保存时锁定一份不可变副本)"
    )
