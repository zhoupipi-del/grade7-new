...
class ApprovalNodeSchema(BaseModel):
    ...
    approver_value: str = Field(default="class_teacher", max_length=100,
...
class TenantApprovalChainCreate(BaseModel):
    ...
    nodes: List[ApprovalNodeSchema] = Field(..., min_length=1, max_length=10,
                                            description="审批节点列表")
...
class TenantApprovalChainUpdate(BaseModel):
    """更新审批链（节点变更会触发版本号自增）"""
    chain_name: Optional[str] = Field(default=None, max_length=100)
    nodes: Optional[List[ApprovalNodeSchema]] = Field(default=None, min_length=1, max_length=10)
    description: Optional[str] = Field(default=None, max_length=500)
