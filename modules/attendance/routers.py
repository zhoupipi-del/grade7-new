...
async def get_dashboard(
    ...
    # P1 多租户加固：校验 query param 传入的 grade_id/class_id 归属
    if _gid:
        await verify_entity_ownership(db, Grade, _gid, current_user, '年级不存在')
    if _cid:
        await verify_entity_ownership(db, SchoolClass, _cid, current_user, '班级不存在')
...
# ═══════════════════════════════════════════════════════════════
#  班级历史大盘 (冲正反哺配套 — 按天聚合 ClassHistoryMetric)
# ═══════════════════════════════════════════════════════════════
...
async def get_class_attendance_history(
    ...
    返回 ClassHistoryMetric 列表，含每日出勤率、缺勤数、请假数等。
    供前端绘制考勤走势折线图 + 热力图。
    ...
    RBAC:
      ...
      - class_teacher: 仅本班
    """
    AttendanceService.check_access("view_class_history", current_user)

    # P0 多租户隔离: 校验 class_id 归属
    await verify_entity_ownership(db, SchoolClass, class_id, current_user, "班级不存在")

    # 角色范围限定 — class_teacher 只看本班
    scope = AttendanceService.resolve_scope(current_user)
    if scope["class_id"] and class_id != scope["class_id"]:
        from .exceptions import ScopePermissionDeniedError
        raise ScopePermissionDeniedError("仅可查看本班考勤历史")

    metrics = await AttendanceService.get_class_attendance_history(
        db=db,
        school_id=current_user.school_id,
        class_id=class_id,
        start_date=start_date,
