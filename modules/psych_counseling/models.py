...
from sqlalchemy import (
    Column, BigInteger, String, Integer, DateTime, Text, Boolean, JSON,
)
...
class PsyAppointment(Base, SchoolMixin):
    ...
    reason_summary = Column(
        String(200), comment="申请理由摘要(脱敏后可展示)",
    )
    status = Column(
...
class PsyConsultRecord(Base, SchoolMixin):
    ...
    encrypted_clog = Column(
        Text, comment="Fernet 加密的咨询日志正文 — 仅 counselor+MS_ADMIN 可解密",
    )
    # ── 明文元数据 (用于索引/统计但不泄露隐私) ──
    risk_level = Column(
        String(10), default="green",
    ...
    consult_category = Column(
        String(30), comment="咨询分类: emotion/interpersonal/academic/family/self_harm/other",
    )
