...
from .schemas import (
    ...
    AutoLocateRequest, TimelineCommentCreate,
)
...
async def auto_locate(
    db: AsyncSession, school_id: int, class_id: int, occurred_at: datetime,
) -> dict:
    """
    ...
    ctx = await TimetableEnricher.enrich_telemetry_event(
        school_id, class_id, occurred_at, db,
    )
    ...
    # 反查科目信息 (subject_id -> subject_code + subject_name)
    subject_id = ctx.get("subject_id")
    subject_code = None
    subject_name = None
    if subject_id:
        subj_result = await db.execute(
            select(GradeSubject.code, GradeSubject.name).where(
                and_(
        ...
        if subj_row:
            subject_code = subj_row[0]
            subject_name = subj_row[1]

    # 前端兼容字段映射
    result["matched"] = result["in_lesson"]
    result["message"] = result["context_desc"] if not result["in_lesson"] else None
    period_idx = result.get("period_index")
    result["slot_number"] = (period_idx + 1) if period_idx is not None else None
...
async def add_timeline_comment(
    db: AsyncSession, school_id: int, obs_id: int,
    user_id: int, data: TimelineCommentCreate,
) -> Optional[dict]:
    """
