...
class PsychClassPortrait(Base, SchoolMixin):
    """
    班级心理画像表

    存储每个班级的心理健康聚合数据：总人数、风险分布、十维度均分。
    portrait_data JSON 结构:
      {
        "total_students": 64,
        "risk_distribution": {"low": 55, "medium": 9, "high": 0},
        "average_score": 89.3,
        "dimension_averages": {"强迫症状": 9.3, ...}
      }
    """
    __tablename__ = "psych_class_portrait"

    id = Column(BigInteger, primary_key=True, autoincrement=True)
    class_id = Column(BigInteger, nullable=False, index=True)
    grade_id = Column(BigInteger, nullable=False, index=True)
    portrait_data = Column(JSON, comment="班级聚合画像数据")
    updated_at = Column(DateTime, default=get_local_now, onupdate=get_local_now)


__all__ = __all__ + ["PsychClassPortrait"]
