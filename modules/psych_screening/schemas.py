...
class PsychDashboardResponse(BaseModel):
    ...
    risk_distribution_map: dict = {}  # {high, medium, low} — 原始对象
    risk_distribution: list = []  # [{level, count, percentage, color}] — 饼图用
    assessment_stats: dict = {}   # {total, need_intervention}
    intervention_stats: dict = {} # {total, tracking, completed}
    intervention_summary: dict = {}  # {total, pending, in_progress, completed, crisis_count}
    dimension_alerts: List[dict] = []  # 维度预警
    dimension_ranking: list = []  # [{code, name, avg_score, deviation_pct}] — 排行用
    overview: dict = {}  # {total_surveys, high_risk_count, medium_risk_count, low_risk_count, critical_count}
