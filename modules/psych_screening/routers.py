...
async def get_dashboard(
    ...
    ss = stats.get("survey_stats", {})
    iv = stats.get("intervention_stats", {})

    total_mssmhs = ss.get("mssmhs_count", 0)
    high_c = int(rd.get("high", 0))
    med_c = int(rd.get("medium", 0))
    low_c = int(rd.get("low", 0))
    total_risk = max(high_c + med_c + low_c, 1)

    # overview — 概览卡片
    stats["overview"] = {
        "total_surveys": ss.get("total", 0),
        "high_risk_count": high_c,
        "medium_risk_count": med_c,
        "low_risk_count": low_c,
        "critical_count": 0,
        "mssmhs_count": total_mssmhs,
        "pce_count": ss.get("pce_count", 0),
    }

    # risk_distribution — 饼图数组格式
    stats["risk_distribution"] = [
        {"level": "高风险", "count": high_c, "percentage": round(high_c/total_risk*100, 1), "color": "#f85149"},
        {"level": "中风险", "count": med_c, "percentage": round(med_c/total_risk*100, 1), "color": "#d29922"},
        {"level": "低风险", "count": low_c, "percentage": round(low_c/total_risk*100, 1), "color": "#3fb950"},
    ]
    stats["risk_distribution_map"] = rd

    # intervention_summary
    stats["intervention_summary"] = {
        "total": iv.get("total", 0),
        "pending": iv.get("total", 0) - iv.get("tracking", 0) - iv.get("completed", 0),
        "in_progress": iv.get("tracking", 0),
        "completed": iv.get("completed", 0),
        "crisis_count": 0,
    }

    # dimension_ranking — 从dimension_alerts衍生
    stats["dimension_ranking"] = []
    for da in stats.get("dimension_alerts", []):
        stats["dimension_ranking"].append({
            "code": da.get("dimension", ""),
            "name": da.get("dimension", ""),
            "avg_score": da.get("average", 0),
            "deviation_pct": min(round(da.get("average", 0) / 30 * 100, 1), 100),
        })
...
from modules.psych_screening.models import PsychClassPortrait
from sqlalchemy import text as sa_text
from pydantic import BaseModel as PydanticBase, Field


class ClassPortraitOut(PydanticBase):
    id: int
    class_id: int
    class_name: str = ""
    grade_id: int
...
class CrossAnalysisOut(PydanticBase):
    id: int
    student_id: int
    student_name: str = ""
    class_name: str = ""
...
async def list_class_portraits(
    ...
    stmt = (
        ...
        .where(*conditions)
        .order_by(PsychClassPortrait.class_id)
    )
    result = await db.execute(stmt)
    portraits = result.scalars().all()

    # 获取班级名称映射
    class_ids = [p.class_id for p in portraits]
    class_map = {}
    if class_ids:
        from core.models import Class as ClassModel
        cls_result = await db.execute(
            select(ClassModel.id, ClassModel.name).where(ClassModel.id.in_(class_ids))
        )
        class_map = {row[0]: row[1] for row in cls_result.all()}

    return [
        ClassPortraitOut(
            id=p.id,
            class_id=p.class_id,
            class_name=class_map.get(p.class_id, f"班级{p.class_id}"),
            grade_id=p.grade_id,
            portrait_data=p.portrait_data or {},
...
async def list_cross_analyses(
    ...
    query_sql = """
        ...
        FROM psych_cross_analyses pca
        JOIN students s ON pca.student_id = s.id
        JOIN classes c ON s.class_id = c.id
        WHERE pca.school_id = :school_id
    """
    params = {"school_id": current_user.school_id}
    if analysis_type:
        query_sql += " AND pca.analysis_type = :analysis_type"
        params["analysis_type"] = analysis_type
    query_sql += " ORDER BY pca.id"

    result = await db.execute(sa_text(query_sql), params)
    ...
    return [
        CrossAnalysisOut(
            ...
            class_name=row[3] or "",
            analysis_type=row[4],
            details_json=row[5] if isinstance(row[5], dict) else {},
